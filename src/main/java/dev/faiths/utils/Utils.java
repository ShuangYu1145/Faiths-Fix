...
public class Utils {
    ...
    public static Block getBlockFromName(String name) {
        return Block.blockRegistry.getObject(new ResourceLocation("minecraft:" + name));
    }
    ...
    public static boolean inFov(Entity viewPoint, float fov, final double posX, final double posZ) {
        fov *= 0.5;
        final double wrapAngleTo180_double = MathHelper.wrapAngleTo180_double((viewPoint.rotationYaw - RotationUtil.angle(posX, posZ)) % 360.0f);
    ...
    public static void sendMessage(String txt) {
        if (nullCheck()) {
            String m = formatColor("&7[&dF&7]&r " + txt);
    ...
    public static void sendDebugMessage(String message) {
        if (nullCheck()) {
            mc.thePlayer.addChatMessage(new ChatComponentText("§7[§dF§7]§r " + message));
        }
    }
    ...
    public static boolean blockAbove() {
        return !(BlockUtil.getBlock(new BlockPos(mc.thePlayer.posX, mc.thePlayer.posY + 2, mc.thePlayer.posZ)) instanceof BlockAir);
    }
    ...
    public static boolean isPlaceable(BlockPos blockPos) {
        return BlockUtil.replaceable(blockPos) || BlockUtil.isFluid(BlockUtil.getBlock(blockPos));
    }

    public static boolean spectatorCheck() {
        return mc.thePlayer.inventory.getStackInSlot(8) != null && mc.thePlayer.inventory.getStackInSlot(8).getDisplayName().contains("Return") || Utils.stripString((mc.ingameGUI).displayedTitle).contains("YOU DIED");
    }
