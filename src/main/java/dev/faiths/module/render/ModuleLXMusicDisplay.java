...
public class ModuleLXMusicDisplay extends CheatModule {
    ...
    public void draw() {
        ...
        if (picurl != null || name != null || lyric != null || progress != null || duration != null) {
            ...
            //歌名
            FontManager.sf24.drawString(name, xValue.getValue() + 65, yValue.getValue(), -1);
            //歌词
            FontManager.sf20.drawString(lyric, xValue.getValue() + 65, yValue.getValue() + FontManager.sf24.getHeight() + 5, -1);
            ...
            //绘制背景条（duration），灰色
            RoundedUtil.drawRound(xValue.getValue(), yValue.getValue() + 60 + 5, durationWidth, 4, 2, Color.GRAY);
            //绘制进度条（progress），白色
            RoundedUtil.drawRound(xValue.getValue(), yValue.getValue() + 60 + 5, progressWidth, 4, 2, Color.WHITE);
