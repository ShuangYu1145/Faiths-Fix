...
public class Minecraft implements IThreadListener, IPlayerUsage
{
    ...
    public Minecraft(GameConfiguration gameConfig)
    {
        ...
        this.displayWidth = Integer.parseInt("1280");
        this.displayHeight = Integer.parseInt("720");
    ...
    private void createDisplay() throws LWJGLException
    {
        ...
        Display.setTitle(Faiths.NAME + " " + Faiths.VERSION);
     ...
     */ public void rightClickMouse()
    {
        if (!this.playerController.getIsHittingBlock())
        {
            ...
            if (Faiths.moduleManager.getModule(ModuleFastPlace.class).getState()) {
    ...
    public void runTick() throws IOException
    {
        ...
        if (!Faiths.INSTANCE.taskManager.getFutureTasks().isEmpty()) {
            Faiths.INSTANCE.getTaskManager().getFutureTasks().forEach(task -> {
                if (task.delay()) {
                    task.execute();
                    Faiths.INSTANCE.getTaskManager().getFutureTasks().remove(task);
                } else {
                    task.run();
                }
            });
        ...
        if (this.thePlayer != null) {
            Faiths.INSTANCE.getEventManager().callEvent(new TickUpdateEvent());
            Faiths.hackerDetector.onTick(0);
        }
        ...
        if (this.currentScreen == null || this.currentScreen.allowUserInput)
        {
            ...
            while (Keyboard.next())
            {
                ...
                if (Keyboard.getEventKeyState())
                {
                    ...
                    {
                        Faiths.INSTANCE.getEventManager().callEvent(new KeyEvent(Keyboard.getEventKey() == 0 ? Keyboard.getEventCharacter() + 256 : Keyboard.getEventKey()));
                        if (k == 1)
                        {
            ...
            Faiths.INSTANCE.getEventManager().callEvent(blockPlaceEvent);
        ...
        Faiths.hackerDetector.onTick(1);
        Faiths.INSTANCE.getEventManager().callEvent(new PostUpdateEvent());
    ...
    {
        Faiths.INSTANCE.getEventManager().callEvent(new WorldEvent(worldClientIn));
        ...
        if (worldClientIn != null)
        {
            ...
            Faiths.INSTANCE.getEventManager().callEvent(new WorldLoadEvent());
        }
        else
        {
    ...
    public void toggleFullScreen(boolean goFullScreen) {
        toggleFullScreen(goFullScreen, 0);
    }
    ...
    public void toggleFullScreen(boolean goFullScreen, int desiredMonitor) {
        ...
        if(goFullScreen == false && !this.fullscreen) {
            newBounds = currentCoordinates;
            _savedWindowedBounds = currentCoordinates;
        }

        try {
            if( this.gameSettings.fullScreen != fullscreen) {
                this.gameSettings.fullScreen = fullscreen;
                this.gameSettings.saveOptions();
            }
            ...
            this.resize((int) newBounds.getWidth(), (int) newBounds.getHeight());
            Display.setVSyncEnabled(this.gameSettings.enableVsync);
            this.updateDisplay();
    ...
    private Rectangle getAppropriateScreenBounds(Rectangle currentCoordinates, int desiredMonitor)
    {
        ...
        if(desiredMonitor < 0 || desiredMonitor == 0) {
