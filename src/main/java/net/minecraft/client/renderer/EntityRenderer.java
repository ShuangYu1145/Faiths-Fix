...
import dev.faiths.module.combat.ModuleGhostHand;
import dev.faiths.module.render.ModuleCamera;
import dev.faiths.module.render.ModuleMotionBlur;
import dev.faiths.ui.notifiction.NotificationRenderer;
...
public class EntityRenderer implements IResourceManagerReloadListener
{
    ...
    public void removeEntities(List<Entity> list) {
        ...
        if (Faiths.moduleManager.getModule(ModuleGhostHand.class).getState()) {
            list.removeIf(ModuleGhostHand::shouldHitThrough);
        }
    ...
    private void hurtCameraEffect(float partialTicks)
    {
        ...
        {
            if(Faiths.moduleManager.getModule(ModuleCamera.class).getState() && Faiths.moduleManager.getModule(ModuleCamera.class).getNoHurtCam())return;
    ...
    private void orientCamera(float partialTicks)
    {
            ...
            else
            {
                ...
                for (int i = 0; i < 8; ++i)
                {
                    ...
                    if (movingobjectposition != null)
                    {
                        ...
                        if (d7 < d3) {
                            if (!Faiths.moduleManager.getModule(ModuleCamera.class).getState() ||
                                    !Faiths.moduleManager.getModule(ModuleCamera.class).getViewClip()) {
                                d3 = d7;
                            }
                        }
                    }
                ...
                GlStateManager.translate(0.0F, 0.0F, (float) -d3 * (Faiths.moduleManager.getModule(ModuleCamera.class).getState() ? Faiths.moduleManager.getModule(ModuleCamera.class).dist.getValue() : 1f));
        ...
        if (Reflector.EntityViewRenderEvent_CameraSetup_Constructor.exists())
        {
            ...
            if (Faiths.moduleManager.getModule(ModuleCamera.class).getState() && Faiths.moduleManager.getModule(ModuleCamera.class).getMcLock() && Mouse.isButtonDown(2)) {
    ...
    public void updateCameraAndRender(float partialTicks, long nanoTime)
    {
        ...
        NotificationRenderer.draw(new ScaledResolution(mc));
    ...
    private void renderWorldPass(int pass, float partialTicks, long finishTimeNano)
    {
        ...
        Faiths.INSTANCE.getEventManager().callEvent(new Render3DEvent(partialTicks));
        ModuleMotionBlur motionBlur = Faiths.moduleManager.getModule(ModuleMotionBlur.class);
