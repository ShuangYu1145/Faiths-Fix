...
public class NetHandlerPlayClient implements INetHandlerPlayClient
{
    ...
    public void handleJoinGame(S01PacketJoinGame packetIn)
    {
        ...
        if(Faiths.moduleManager.getModule(ModuleProtocol.class).getState()){
    ...
    public void handleSpawnGlobalEntity(S2CPacketSpawnGlobalEntity packetIn)
    {
        ...
        if (entity != null)
        {
            entity.lastPacketPosX = entity.serverPosX;
            entity.lastPacketPosY = entity.serverPosY;
            entity.lastPacketPosZ = entity.serverPosZ;
    ...
    public void handleEntityMetadata(S1CPacketEntityMetadata packetIn)
    {
        ...
        List<DataWatcher.WatchableObject> watchableObjects = packetIn.func_149376_c();
        if (entity != null && watchableObjects != null) {
            if (entity instanceof EntityLivingBase &&
                    !(entity instanceof EntityPlayerSP)) {
                DataWatcher.WatchableObject watchableObject = watchableObjects.get(0);
                if (watchableObject.getDataValueId() == 6 && watchableObject.getObjectType() == 3) {
                    ...
                    float dif = health - newHealth;
                    if (dif >= 0.0F && newHealth != 10.0F)
                        Faiths.INSTANCE.getEventManager().callEvent(new EntityHealthUpdateEvent(elb, dif));
                }
            }
    ...
    public void handlePlayerPosLook(S08PacketPlayerPosLook packetIn)
    {
        ...
        if (Faiths.moduleManager.getModule("NoRotateSet").getState()) {
            entityplayer.setPositionAndRotation(d0, d1, d2, entityplayer.rotationYaw, entityplayer.rotationPitch);
            this.netManager.sendPacket(new C03PacketPlayer.C06PacketPlayerPosLook(entityplayer.posX,
                    entityplayer.getEntityBoundingBox().minY, entityplayer.posZ, f,
                    f1, false));
        } else {
