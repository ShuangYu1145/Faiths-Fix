...
public abstract class EntityLivingBase extends Entity
{
    ...
    private int getArmSwingAnimationEnd() {
        final float slowValue = ModuleAnimation.swingSpeed.getValue();

        return this.isPotionActive(Potion.digSpeed) ? 6 - (1 + this.getActivePotionEffect(Potion.digSpeed).getAmplifier()) : (int) ((this.isPotionActive(Potion.digSlowdown)
     ...
     */
    protected void jump() {
        JumpEvent jumpEvent = new JumpEvent(this.movementYaw, this.getJumpUpwardsMotion());
        ...
        if (this instanceof EntityPlayerSP) {
            Faiths.INSTANCE.getEventManager().callEvent(jumpEvent);
    ...
    public void onLivingUpdate()
    {
        ...
        if (Faiths.moduleManager.getModule("NoJumpDelay") != null && Minecraft.getMinecraft().thePlayer != null) {
            if (Faiths.moduleManager.getModule("NoJumpDelay").getState()) {
                this.jumpTicks = 0;
            }
        }
